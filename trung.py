...
#         return super().update_state(y_true, tensorflow.where(y_pred < 0.5, 0, 1), sample_weight)
...
#               metrics=[MyMeanIoU(num_classes=2)])
...
#     Data_file = os.path.join(DataPath, file)
#     # print(Data_file)
#     img = cv2.imread(Data_file, 1)
#     img = cv2.resize(img, dsize=(640, 160))
#     X.append(img)

# for file in os.listdir(LabelsPath):
#     Data_file = os.path.join(LabelsPath, file)
#     # print(Data_file)
#     img = cv2.imread(Data_file, 0)
#     img = cv2.resize(img, dsize=(640, 160))
#     Y.append(img)
